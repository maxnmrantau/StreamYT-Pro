...
[Fitur Utama](#-fitur-utama) • [Cara Instalasi](#-cara-instalasi--menjalankan) • [Panduan Penggunaan](#-panduan-penggunaan) • [Mode Durasi](#-penjelasan-mode-durasi-a-b-c) • [Notifikasi Telegram](#-panduan-notifikasi-telegram) • [System Tray](#-system-tray--shortcut-taskbar) • [Lisensi](#-lisensi)
...
**StreamYT Pro** adalah aplikasi server lokal berbasis **FastAPI** dan **FFmpeg** yang dirancang khusus untuk kreator konten, podcaster, dan pengelola channel YouTube yang ingin menyiarkan video rekaman ke YouTube Live secara otomatis dan terjadwal 24/7 langsung dari PC Windows mereka.

Tidak seperti OBS Studio yang berat dan memerlukan konfigurasi manual setiap sesi, **StreamYT Pro**:
* Berjalan sangat ringan di latar belakang (*background system tray*).
* Memiliki penjadwal otomatis berbasis waktu lokal PC.
* Mendukung **Stream Key unik yang berbeda untuk setiap video/sesi**.
* Mengirimkan notifikasi langsung ke akun **Telegram** Anda saat siaran dimulai, selesai, atau terjadi gangguan.
* Dilengkapi antarmuka web dashboard interaktif dengan tema *dark glassmorphism*.
...
- 🔲 **System Tray Icon (Senyap & Bersih)**: Dilengkapi dengan ikon tray di taskbar pojok kanan bawah Windows. Server dapat berjalan 100% senyap di latar belakang tanpa jendela CMD yang mengganggu layar.
...
- 📂 **Dual-Pane File Explorer Web**: Memilih file video PC langsung dari antarmuka web yang intuitif dengan breadcrumb interaktif, filter pencarian live, dan pintasan cepat folder (Videos, Downloads, Documents, Drives).
...
* **OS & Tray Integration**: `pystray`, `Pillow`, Windows Win32 API (`ctypes`)
...
1. **Python 3.10+** (Centang *"Add Python to PATH"* saat instalasi).
...
1. **Buka Dashboard**: Akses **`http://localhost:8000`** di browser Anda.
2. **Tambah Jadwal Sesi**:
   * Klik tombol **`+ Tambah Jadwal Sesi`**.
   * Masukkan **Nama Sesi**.
   * Klik **`Pilih File Video`** untuk memilih berkas video dari komputer Anda.
...
├── Jalankan_Tanpa_CMD.vbs      # Silent launcher (tanpa jendela CMD)
...
│   └── tray_manager.py         # System Tray & window visibility manager
├── data/
